...
class InteractiveTester:
    ...
    async def initialize(self):
        ...
        if integration_mode != 'real':
            logger.warning("⚠️  Not in real integration mode. Set INTEGRATION_MODE=real for full testing.")
    ...
    async def _verify_platform_connectivity(self):
        ...
        try:
            ...
            logger.error("   cd platform && python3 -m uvicorn src.main:app --reload --host 0.0.0.0 --port 8000")
            raise
    ...
    async def _execute_mcp_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            async with self.session.post(
                ...
                timeout=aiohttp.ClientTimeout(total=120)  # 2 minute timeout for long operations
            ) as response:
                if response.status == 200:
    ...
    async def _test_create_muppet(self):
        ...
        response = await self._execute_mcp_tool("create_muppet", {
            "name": muppet_name,
            "template": template
        })
        ...
        if response.get("success"):
            ...
            print(f"❌ Failed to create muppet: {response.get('error', 'Unknown error')}")
    ...
    async def _test_get_muppet_status(self):
        ...
        response = await self._execute_mcp_tool("get_muppet_status", {
            "name": muppet_name
        })
    ...
    async def _show_manual_deletion_instructions(self):
        ...
        print("   aws ssm delete-parameter --name /muppet-platform/<muppet-name>/config")
        print()
        ...
        print()
        print("⚠️  **Important:** Always verify what you're deleting before running commands!")
    ...
    async def _test_pipeline_management(self):
        ...
        if choice == '1':
            ...
            response = await self._execute_mcp_tool("list_workflow_versions", {
            ...
            response = await self._execute_mcp_tool(tool_name, {
            ...
            if response.get("success"):
                ...
                print(f"❌ Pipeline operation failed: {response.get('error', 'Unknown error')}")
    ...
    async def _test_steering_docs(self):
        ...
        if choice == '1':
            ...
            if response.get("success"):
                ...
                print(f"❌ Steering update failed: {response.get('error', 'Unknown error')}")
