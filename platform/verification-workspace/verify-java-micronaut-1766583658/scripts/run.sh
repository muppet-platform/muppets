...
# verify-java-micronaut-1766583658 Muppet - Run Script
# Starts the muppet locally for development and testing

set -e

echo "🚀 Starting verify-java-micronaut-1766583658 muppet locally..."
...
# Option 2: Run with Docker (production-like)
    ...
    if ! docker images verify-java-micronaut-1766583658:latest -q | grep -q .; then
    ...
    docker run --rm \
        -p 3000:3000 \
        --env-file .env.local \
        --name verify-java-micronaut-1766583658-dev \
        verify-java-micronaut-1766583658:latest
...
# Default: Run JAR directly
    ...
    if [ ! -f build/libs/verify-java-micronaut-1766583658-*-all.jar ]; then
        echo "📦 Building JAR first..."
        ./gradlew shadowJar
    fi
    
    # Find the JAR file
    JAR_FILE=$(find build/libs -name "verify-java-micronaut-1766583658-*-all.jar" | head -n 1)
