...
# verify-java-micronaut-1766638048 Muppet - Local Development Environment Setup
# This script sets up the local development environment including Rancher Desktop

set -e

echo "🚀 Setting up local development environment for verify-java-micronaut-1766638048..."
...
if [ ! -f .env.local ]; then
    ...
    cat > .env.local << EOF
# Local development environment variables for verify-java-micronaut-1766638048
MICRONAUT_ENVIRONMENTS=development
MUPPET_NAME=verify-java-micronaut-1766638048
...
if [ -f "./gradlew" ]; then
    ...
    if ./gradlew --version >/dev/null 2>&1; then
        ...
        echo "  1. Corrupted gradle-wrapper.jar file (common with Git binary file issues)"
        ...
        if [ -f "gradle/wrapper/gradle-wrapper.jar" ]; then
            echo "✅ gradle-wrapper.jar exists ($(stat -f%z gradle/wrapper/gradle-wrapper.jar 2>/dev/null || stat -c%s gradle/wrapper/gradle-wrapper.jar 2>/dev/null || echo "unknown") bytes)"
            ...
            if file gradle/wrapper/gradle-wrapper.jar | grep -q "Zip archive"; then
                ...
                echo "   This often happens when Git corrupts binary files during clone/pull operations."
                
                # Check if it looks like a text file (Git corruption symptom)
                if head -c 100 gradle/wrapper/gradle-wrapper.jar | grep -q "^PK"; then
                    echo "   File starts with ZIP signature - may be partially corrupted"
                else
                    echo "   File doesn't start with ZIP signature - likely corrupted by Git"
                    echo "   First 50 bytes:"
                    head -c 50 gradle/wrapper/gradle-wrapper.jar | hexdump -C | head -3
                fi
            fi
        else
        ...
        # Attempt automatic fix for corrupted wrapper
        echo "🔧 Attempting to fix Gradle wrapper..."
        
        # Check if we have gradle installed globally to regenerate wrapper
        if command_exists gradle; then
            echo "   Using global Gradle installation to regenerate wrapper..."
            
            # Remove corrupted wrapper files
            ...
            if ./gradlew --version >/dev/null 2>&1; then
                ...
                echo "   This was likely caused by Git corrupting the binary gradle-wrapper.jar file."
                echo "   Consider adding '*.jar binary' to your .gitattributes file to prevent this."
            else
                echo "❌ Failed to regenerate Gradle wrapper even with global Gradle"
                echo ""
            ...
            echo ""
            echo "The gradle-wrapper.jar file appears to be corrupted, likely by Git."
            echo "This is a common issue when Git treats binary files as text files."
            echo ""
            ...
            echo "  2. Remove corrupted wrapper: rm -rf gradle/wrapper"
            echo "  3. Regenerate wrapper: gradle wrapper --gradle-version 8.5"
            echo "  4. Make executable: chmod +x ./gradlew"
            echo "  5. Test: ./gradlew --version"
            echo ""
            echo "To prevent this in the future:"
            echo "  - Add '*.jar binary' to your .gitattributes file"
            echo "  - Ensure Git is configured properly for binary files"
