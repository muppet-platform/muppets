...
replace_template_vars() {
    local file=$1
    if [ -f "$file" ]; then
        sed -i.bak "s/verify-java-micronaut-1766586224/${TEST_MUPPET_NAME}/g" "$file"
        rm -f "${file}.bak"
    fi
}
...
template_files=(
    "src/main/java/com/muppetplatform/verify-java-micronaut-1766586224/Application.java"
    "src/main/java/com/muppetplatform/verify-java-micronaut-1766586224/controller/HealthController.java"
    "src/main/java/com/muppetplatform/verify-java-micronaut-1766586224/controller/ApiController.java"
...
for file in "${template_files[@]}"; do
    if [ -f "$file" ]; then
        if grep -q "verify-java-micronaut-1766586224" "$file"; then
...
if [ -d "src/main/java/com/muppetplatform/verify-java-micronaut-1766586224" ]; then
    mv "src/main/java/com/muppetplatform/verify-java-micronaut-1766586224" "src/main/java/com/muppetplatform/${TEST_MUPPET_NAME}"
fi

if [ -d "src/test/java/com/muppetplatform/verify-java-micronaut-1766586224" ]; then
    mv "src/test/java/com/muppetplatform/verify-java-micronaut-1766586224" "src/test/java/com/muppetplatform/${TEST_MUPPET_NAME}"
fi
...
for java_file in "${java_files[@]}"; do
    if [ -f "$java_file" ]; then
        if grep -q "$TEST_MUPPET_NAME" "$java_file" && ! grep -q "verify-java-micronaut-1766586224" "$java_file"; then
...
if [ -f "Dockerfile" ]; then
    if grep -q "${TEST_MUPPET_NAME}" "Dockerfile" && ! grep -q "verify-java-micronaut-1766586224" "Dockerfile"; then
...
for script_file in "${script_files[@]}"; do
    if [ -f "$script_file" ]; then
        if grep -q "$TEST_MUPPET_NAME" "$script_file" && ! grep -q "verify-java-micronaut-1766586224" "$script_file"; then
...
find . -type f \( -name "*.java" -o -name "*.gradle*" -o -name "*.yml*" -o -name "*.yaml*" -o -name "*.md" -o -name "*.sh" -o -name "Dockerfile*" \) -print0 | while IFS= read -r -d '' file; do
    if [ -f "$file" ]; then
        sed -i.bak "s/verify-java-micronaut-1766586224/${SPECIAL_TEST_NAME}/g" "$file"
        rm -f "${file}.bak"
    fi
done
