...
main() {
    ...
    find . -name "*.template" | while read -r file; do
        sed "s/verify-java-micronaut-1766584181/${TEST_NAME}/g" "$file" > "${file%.template}"
        rm "$file"
    done
    find . -type f \( -name "*.java" -o -name "*.yml" -o -name "Dockerfile" \) -exec sed -i.bak "s/verify-java-micronaut-1766584181/${TEST_NAME}/g" {} \;
