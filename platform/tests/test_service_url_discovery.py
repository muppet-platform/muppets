...
class TestServiceUrlDiscovery:
    ...
    def test_service_url_outputs_generation(self, outputs_tf_template):
        ...
        assert 'output "load_balancer_dns"' in content, "Should output load balancer DNS"
        assert 'value       = aws_lb.main.dns_name' in content, "Should reference ALB DNS name"
        assert 'description = "Load balancer DNS name (for easy copy/paste)"' in content, "Should describe DNS output"
    ...
    def test_application_url_output(self, outputs_tf_template):
        ...
        assert 'value       = "http://${aws_lb.main.dns_name}"' in content, "Should construct HTTP URL"
        assert 'description = "HTTP URL to access the application"' in content, "Should describe application URL"
    ...
    def test_health_check_url_output(self, outputs_tf_template):
        ...
        assert 'value       = "http://${aws_lb.main.dns_name}/health"' in content, "Should construct health check URL"
        assert 'description = "Health check endpoint URL"' in content, "Should describe health check URL"
    ...
    def test_deployment_info_output(self, outputs_tf_template):
        ...
        assert 'description = "Deployment information for CI/CD pipelines"' in content, "Should describe deployment info"
        
        # Check for deployment info structure
        assert 'service_name = aws_ecs_service.app.name' in content, "Should include service name in deployment info"
        assert 'load_balancer_dns = aws_lb.main.dns_name' in content, "Should include load balancer DNS in deployment info"
        assert 'health_check_path = "/health"' in content, "Should include health check path in deployment info"
        assert 'environment = var.environment' in content, "Should include environment in deployment info"
    ...
    def test_direct_resource_references(self, outputs_tf_template):
        ...
        assert 'aws_lb.main.dns_name' in content, "Should reference ALB resource directly"
        assert 'aws_ecs_service.app.name' in content, "Should reference ECS service directly"
    ...
    def test_output_descriptions_are_helpful(self, outputs_tf_template):
        ...
        for description in descriptions:
            assert description in content, f"Should include helpful description: {description}"
    ...
    def test_output_syntax_validation(self, outputs_tf_template):
        ...
        for output_name in ['load_balancer_dns', 'application_url', 'health_check_url', 'deployment_info']:
            assert f'output "{output_name}"' in content, f"Should contain {output_name} output"
    ...
    def test_output_comments_and_documentation(self, outputs_tf_template):
        ...
        assert '# Simplified outputs for {{muppet_name}}' in content, "Should include header with muppet name"
        assert '# Provides essential information for developers and CI/CD' in content, "Should explain purpose"
