...
class TestSimplifiedTerraformConfig:
    ...
    def test_terraform_main_template_exists(self, terraform_template_dir):
        ...
        template_path = terraform_template_dir / "main.tf.template"
        assert template_path.exists(), "Simplified terraform main.tf.template should exist"
    ...
    def test_terraform_validation_syntax(self, main_tf_template):
        ...
        assert 'required_version = ">= 1.5"' in content, "Should specify minimum terraform version"
        assert 'required_providers {' in content, "Should specify required providers"
        
        # Check for AWS provider
        assert 'source  = "hashicorp/aws"' in content, "Should use hashicorp AWS provider"
    ...
    def test_default_vpc_usage(self, main_tf_template):
        ...
        assert 'data "aws_vpc" "default"' in content, "Should use default VPC data source"
        assert 'default = true' in content, "Should specify default VPC"
        
        # Check for default subnets data source
        assert 'data "aws_subnets" "default"' in content, "Should use default subnets data source"
    ...
    def test_ecr_data_source_approach(self, main_tf_template):
        ...
        assert 'data "aws_ecr_repository" "main"' in content, "Should use ECR data source"
        assert 'resource "aws_ecr_repository"' not in content, "Should NOT create ECR resource"
        
        # Should reference data source in task definition
        assert 'data.aws_ecr_repository.main.repository_url' in content, "Should reference ECR data source"
        
        # Should include comment explaining the approach
        assert 'CD workflow step "Ensure ECR repository exists"' in content, "Should document ECR creation approach"
    ...
    def test_arm64_architecture_config(self, main_tf_template):
        ...
        assert 'cpu_architecture        = "ARM64"' in content, "Should use ARM64 architecture"
        
        # Check for ARM64 comment
        assert 'Use ARM64 architecture for better cost/performance' in content, "Should document ARM64 benefits"
    ...
    def test_java21_environment_variables(self, main_tf_template):
        ...
        assert '"JAVA_VERSION"' in content, "Should set JAVA_VERSION environment variable"
        assert 'value = "21"' in content, "Should set Java version to 21"
        assert '"JAVA_DISTRIBUTION"' in content, "Should set JAVA_DISTRIBUTION environment variable"
        ...
        assert 'EnableDynamicAgentLoading' in content, "Should enable dynamic agent loading"
    ...
    def test_complexity_reduction_validation(self, main_tf_template):
        ...
        assert len(lines) < 400, f"Simplified config should be under 400 lines, got {len(lines)}"
        assert len(lines) > 250, f"Config should have substantial content, got {len(lines)}"
    ...
    def test_common_tags_configuration(self, main_tf_template):
        ...
        assert 'JavaVersion   = "21-LTS"' in content, "Should tag with Java 21 LTS version"
    ...
    def test_health_check_configuration(self, main_tf_template):
        ...
        assert 'curl -f http://localhost:3000/health' in content, "Should check health endpoint"
    ...
    def test_auto_scaling_configuration(self, main_tf_template):
        ...
        assert 'resource "aws_appautoscaling_target" "ecs_target"' in content, "Should define auto-scaling target"
        
        # Check for CPU-based scaling
        assert 'resource "aws_appautoscaling_policy" "ecs_policy_cpu"' in content, "Should define CPU scaling policy"
        assert 'ECSServiceAverageCPUUtilization' in content, "Should use CPU utilization metric"
        
        # Check for memory-based scaling
        assert 'resource "aws_appautoscaling_policy" "ecs_policy_memory"' in content, "Should define memory scaling policy"
        assert 'ECSServiceAverageMemoryUtilization' in content, "Should use memory utilization metric"
