...
""",
    ...
    async def test_workflow_files_are_generated_locally(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert "security.yml" in workflow_names, "Security workflow should be generated"
            
            # Verify workflow content contains Java 21 LTS enforcement
            ci_content = (workflows_dir / "ci.yml").read_text()
            assert "java-version: '21'" in ci_content, "CI workflow should enforce Java 21 LTS"
            assert "distribution: 'corretto'" in ci_content, "CI workflow should use Amazon Corretto"

    @pytest.mark.asyncio
    async def test_push_template_code_includes_workflows(self, template_files_with_workflows):
...
#     @pytest.mark.asyncio
    async def test_workflow_content_enforces_java_21_lts(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert "distribution: 'corretto'" in ci_content, "Should use Amazon Corretto"
    ...
    async def test_end_to_end_muppet_creation_includes_workflows(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(workflow_files) == 3, f"Should include 3 workflow files, got {len(workflow_files)}: {workflow_files}"
    ...
    @pytest.mark.asyncio
    async def test_mock_mode_simulates_workflow_creation(self, template_files_with_workflows):
    ...
    def test_workflow_files_contain_required_java_21_elements(self, template_files_with_workflows):
        ...
        for workflow_name, workflow_content in [("CI", ci_workflow), ("CD", cd_workflow)]:
            assert "java-version: '21'" in workflow_content, f"{workflow_name} should specify Java 21"
            assert "distribution: 'corretto'" in workflow_content, f"{workflow_name} should use Amazon Corretto"
            assert "Amazon Corretto JDK 21" in workflow_content, f"{workflow_name} should mention Amazon Corretto 21"