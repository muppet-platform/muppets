...
test_template_generation() {
    ...
    find . -type f -name "*.template" | while read -r file; do
        ...
        sed "s/{{muppet_name}}/${TEST_MUPPET_NAME}/g" "$file" > "$new_file"
        rm "$file"
    done
    
    # Substitute in other files
    find . -type f \( -name "*.java" -o -name "*.yml" -o -name "*.yaml" -o -name "*.md" -o -name "Dockerfile" \) -exec sed -i.bak "s/{{muppet_name}}/${TEST_MUPPET_NAME}/g" {} \;
